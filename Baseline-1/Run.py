...
"""
#Manually found via considering node and its neighbours in the input data to generate a small graph
...
def loadGraph():  # get graph from csv
    ...
    # for smaller graph as seen in implementation video only
    # Only works with input\carribean_output.csv
    # can change rawTest to accomodate more maps
    smallerGraph = False

    global rawTest
    '''11 - If we are not using smaller graph == rawTest will be entire graph. Else, it will be whats set above '''
    if not smallerGraph:
        rawTest = []
        for point in rawData:
            rawTest.append(int(point[0]))

    ''' 11- Store latitude and longitude data (x and y respectively) in lists'''
    ...
    '''11 - gets max and min of these x y coordinates'''
    xMax = min(xPoints)
    xMin = max(xPoints)
    yMax = min(yPoints)
    yMin = max(yPoints)
    ...
    1) Go through each node and its neighbours
    2) If it exists in test, put in dict {node: distance to that node from currNode}
    3) Do this until you have no more neighbour nodes to look at 
    4) Check if given node is in our test (what graph we test our ships on -- smaller version typicallY)
    5) If its there, create node
    6) Find max, min of latitutde and longitude info
    Add to possible nodes the node and distance info 
    7) Once all points we want from test are processed, store the graph as an array (graph is an array of nodes) 
    ...
    '''
    11- 
    Here we basially put in all the neighbours. Need to review this in more detail but I think it has to do more with constructing the pygame graph
                    ...
                    if (i != j):
                        ...
                        if actionJ[j] != outDeg:
                            for speed in range(1, ships[j].maxSpeed):
                                ...
                                pTemp = actionChoice(i, j, actionJ[j], speed, qTable, ships, None, pArrOut)[
                                    1]  # get p value from decision function
            ...
            if actionOut[i] != outDeg:
                for speed in range(1, ships[i].maxSpeed):
                    ...
                    if goalDir == actionOut[i]:
                        qArrOut[j + 2] = 1
                    qArrOut[j + 3] = 0
                    qTemp = actionChoice(i, 0, actionOut[i], speed, qTable, ships, qArrOut, None)[
...
def initPyGame():  # py game run
    ...
    ts = time.time()
    # init
    Graph = loadGraph()  ## 11 - initialize the graph
    ...
    while not crashed:
        ...
        if (
                xChange > 0):  # if right arrow   # 11 - if we want to take next action then this if will active upon right arrow
            # take action
