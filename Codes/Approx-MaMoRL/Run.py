...
class Ship:
    ...
    def __init__(self, shipNo, Graph):  # initialize ship members
        global goalNode
        self.Num = shipNo
        # self.assignFixedLocs(Graph, loc=startLoc) #uncomment this line for fixed location and comment out the next line
...
def collisiontDetect(ships):  # if collision occurs, detect it and return true
    for i in range(0, nShip):
...
def calcNewPos(ships, Graph, qTable, t, test=1):  # Take action based on calculations 
    global goalSensed
    if test < 0:  
        a = 0.6
    state = ()
        ...
        for i in noOfShip:
            for j in deg:
                ...
                with open(resultPath, mode='a') as csv_file:
                    ...
                    if csv_file.tell() == 0:
                        writer.writeheader()
                    writer.writerow({'No of Ships': nShip, 'out degree': outDeg, 'Goal Node': goalNode, 'Ship Location': str(locs),
                         'Max time': str(max1), 'Total Fuel': str(sum(fuels)), 'Running time': str(te - ts), 'qSize': str(qSize), 'pSize': str(pSize)})
